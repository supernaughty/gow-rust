...
**Verified:** 2026-04-25T14:10:00Z
**Status:** human_needed
**Re-verification:** Yes — after gap closure by plan 04-10 (tr POSIX character classes)

## Re-Verification Summary

Plan 04-10 closed the one remaining gap from the previous verification (2026-04-25T12:40:02Z): `tr` POSIX character class expansion (`[:alpha:]`, `[:digit:]`, etc.). All 34 truths are now verified. One item requires human verification: `grep --color=always` emitting ANSI codes. That item was carried over from the prior verification and is unchanged.

**Previous score:** 32/34 (gaps_found)
**Current score:** 34/34 (human_needed — all truths verified; one human check remains)

### What was closed

**Truth #4: tr handles character ranges (a-z) and character classes ([:alpha:])**

Plan 04-10 made two changes to `crates/gow-tr/src/lib.rs`:

1. Added `fn expand_posix_class(class: &str, res: &mut Vec<u8>)` at line 154 — a match-dispatch helper with arms for alpha (52 ASCII letters), digit (0-9), lower (a-z), upper (A-Z), space (space/tab/newline/CR/FF/VT), alnum, blank, and xdigit.

2. Inserted a POSIX class detection branch at line 222 in `expand_set` — between the escape-sequence branch and the range branch — that checks for `chars[i] == '[' && chars[i + 1] == ':'`, scans forward for `:]` terminator, extracts the class name, and delegates to `expand_posix_class`. Unterminated `[:` sequences fall through to literal `[` (DoS-safe, bounded scan).

Three integration tests were added to `crates/gow-tr/tests/integration.rs`:
- `test_tr_posix_class_digit` — `tr -dc '[:digit:]'` on `abc123def` → `123`
- `test_tr_posix_class_lower_to_upper` — `tr '[:lower:]' '[:upper:]'` on `hello world` → `HELLO WORLD`
- `test_tr_posix_class_alpha_delete` — `tr -d '[:alpha:]'` on `abc123def` → `123`

`cargo test -p gow-tr` exits 0: 1 unit test + 4 original integration tests + 3 new integration tests = **7 tests pass, 0 fail**.
...
| 3 | tr translates, deletes (-d), and squeezes (-s) characters from stdin | VERIFIED | gow-tr/src/lib.rs 271 lines; fn expand_set present; 7 integration tests pass (4 original + 3 new) |
| 4 | tr handles character ranges (a-z) and character classes ([:alpha:]) | VERIFIED (closed) | fn expand_posix_class at line 154 with arms for alpha/digit/lower/upper/space/alnum/blank/xdigit; [:class:] detection branch at line 222 in expand_set; test_tr_posix_class_digit/lower_to_upper/alpha_delete all pass; cargo test -p gow-tr exits 0 (7/7 tests) |
...
| 22 | sed d command deletes matching lines | VERIFIED | Cmd::Delete variant at line 35; parse_command handles 'd' at line 275; process_content Cmd::Delete branch at line 459; test_sed_delete_all, test_sed_delete_line_number, test_sed_delete_range all pass (15 total tests) |
| 23 | sed address ranges restrict commands to line spans (e.g. 2,5s/a/b/) | VERIFIED | AddrSpec enum with None/Single/Range at line 57; range_active vec at line 407; fn parse_address at line 173; test_sed_address_range_substitute passes |
...
| 29 | sort -k N sorts by the Nth whitespace-delimited field | VERIFIED | struct KeySpec at line 21; -k arg at line 427 (.short('k')); fn extract_key_field at line 129; test_sort_key_field_1 passes |
| 30 | sort -k N,M sorts from field N through field M | VERIFIED | KeySpec.end_field: Option<usize> at line 24; parse_single_key handles comma separator; test_sort_key_separator passes |
...
|----------|-----------|--------------|--------|---------|
| `crates/gow-tr/src/lib.rs` | 150 | 271 | VERIFIED | fn expand_posix_class (lines 154-187); [:class:] detection branch in expand_set (lines 222-242); gow_core::init() present |
| `crates/gow-tr/tests/integration.rs` | — | 75 lines | VERIFIED | 7 tests: translate, delete, squeeze, complement + posix_class_digit, lower_to_upper, alpha_delete |
...
|------|----|-----|--------|---------|
| gow-tr/src/lib.rs expand_set | expand_posix_class | chars[i]=='[' && chars[i+1]==':' branch at line 222 | WIRED | Detection branch scans for ':]' terminator, extracts class name, calls expand_posix_class |
...
|----------|---------|--------|--------|
| tr -dc '[:digit:]' on 'abc123def' | `cargo test -p gow-tr test_tr_posix_class_digit` | test passed | PASS |
| tr '[:lower:]' '[:upper:]' on 'hello world' | `cargo test -p gow-tr test_tr_posix_class_lower_to_upper` | test passed | PASS |
| tr -d '[:alpha:]' on 'abc123def' | `cargo test -p gow-tr test_tr_posix_class_alpha_delete` | test passed | PASS |
| All 7 gow-tr tests (no regression) | `cargo test -p gow-tr` | 7 passed, 0 failed | PASS |

### Requirements Coverage

| Requirement | Plans | Description | Status | Evidence |
|-------------|-------|-------------|--------|---------|
| R008 | 04-05, 04-08 | 줄 정렬 (-n, -r, -u, -k 키필드) | SATISFIED | -n/-r/-u/-f pass; -k with KeySpec/extract_key_field fully implemented; 12 sort tests including 4 key-field tests |
| R009 | 04-02 | 중복 줄 제거/카운트 (-c, -d) | SATISFIED | uniq with -c/-d/-u/-i passes integration tests |
| R010 | 04-02, 04-10 | 문자 변환/삭제 (-d, -s, 문자 클래스) | SATISFIED | tr ranges work; tr character classes ([:alpha:]/[:digit:]/[:lower:]/[:upper:]/[:space:] etc.) implemented via expand_posix_class; cut fully works; 7 tr integration tests pass |
...
No blocker anti-patterns remain. The previous Warning anti-pattern (`expand_set handles no [:class:] notation`) is resolved by plan 04-10.
...
No gaps remain. All 34 truths are verified. The phase goal is achieved: all 9 utilities (grep, sed, sort, uniq, tr, cut, diff, patch, awk) are implemented with high GNU compatibility, Windows-native UTF-8 support, and atomic file operations.

One human verification item remains from the prior verification: `grep --color=always` ANSI output. This is not a code gap — the color emission path is wired and correct — but it lacks an automated test to confirm the actual escape bytes are produced.

---

_Verified: 2026-04-25T14:10:00Z_
